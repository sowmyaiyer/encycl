for (hismod in c("H3K27ac","H3K4me1","H3K4me3","H3K9ac"))
{
	...
	{
		df <- as.data.frame(read.table(paste("numberofpeaks_above_95th_",hismod,".",distance,".celltype_specific.txt",sep=""),sep=" "))
		colnames(df) <- c("cell","percentage")
		pdf(paste(hismod,"_above_95th.",distance,".celltype_specific.pdf",sep=""))
		par(xpd=TRUE)
		par(xaxs="i")
