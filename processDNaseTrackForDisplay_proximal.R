...
df <- as.data.frame(read.table("multi-tissue.master.v2.proximal.bed",sep="\t"))
cat("read file\n")
...
dnase_signal_track <- data.frame(df[,1],df[,2],df[,3],paste("re",gsub(df[,1],pattern="chr",replacement=""),".",floor((df[,2]+df[,3])/2),sep=""),normalized_score_signal, ".", df[,2],df[,3], "51,138,87", unique_cell_types_clean,number_unique_cell_types_merged)

write.table(x=dnase_signal_track, file="dnase.proximal.bed", sep="\t", col.names=FALSE, row.names=FALSE, quote=FALSE)
