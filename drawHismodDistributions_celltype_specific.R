...
pdf(paste("mean_hismods_matched_random_V2_celltype_specific.pdf",sep=""))
for (cell in cells)
{
	cat(cell,"\n")
	par(mfrow=c(4,2))
	...
	for (hismod in hismods)
	{
		if (cell %in% cellTypes_hismods[[as.character(hismod)]])
		{
			...
			{
        			dist_dnase <- as.numeric(scan(paste("/home/si14w/nearline/decorate_dnase/dnase_master_V2/allhismodsignals/",cell,".",hismod,".dnase_peaks.",distance,".",cell,".txt",sep="")))
        			dist_random <- as.numeric(scan(paste("/home/si14w/nearline/decorate_dnase/dnase_master_V2/allhismodsignals/",cell,".",hismod,".randomBg.",distance,".",cell,".txt",sep="")))
